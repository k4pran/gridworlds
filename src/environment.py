...
from src.environment_exceptions import InvalidActionException
...
class Env:
    ...
    def __init__(self, grid, agent_start_pos=None, **kwargs):
    ...
    def get_state_space(self):
        return (tile.get_state_rep() for tile in self._grid.tiles)
    ...
    def _move(self, action):
        # up
        if action == 0 and self._agent_tile.row > 0:
        ...
        #down
        elif action == 1 and self._agent_tile.row < self._grid.rows -1:
            self.agent_pos = (self.agent_pos[0], self.agent_pos[1] + self.tile_height)
            self._agent_tile = self._grid.get_tile(self._agent_tile.row + 1, self._agent_tile.col)

        #left
        elif action == 2 and self._agent_tile.col > 0:
        ...
        #right
        elif action == 3 and self._agent_tile.col < self._grid.cols -1:
            self.agent_pos = (self.agent_pos[0] + self.tile_width, self.agent_pos[1])
            self._agent_tile = self._grid.get_tile(self._agent_tile.row, self._agent_tile.col + 1)
